...
OUTPUT_DIR = Path("/home/s3758869/synchain-absa-emotion/data/input_data/chunks_for_teacher_model_ann")
...
for file_path in INPUT_FILES:
   ...
   positive = data[data["original_sentiment"].str.lower().str.contains("positive")].copy()
   negative = data[data["original_sentiment"].str.lower().str.contains("negative")].copy()
   ...
   while pos_idx < len(positive) or neg_idx < len(negative):
      ...
      if chunk_parts:
         ...
         print(f"Chunk {chunk_idx}: {len(chunk)} samples - {chunk['original_sentiment'].value_counts().to_dict()}")
