...
def parse_aspect_extraction(text):
    ...
    for line in lines:
        if any(
            keyword in line.lower() for keyword in ["aspect:", "aspects:", "-", "*"]
        ):
...
def knowledge_distillation(
    ...
    for i, sentence in enumerate(tqdm.tqdm(original_sentences)):
        ...
        response = generate_response(
            student_model, student_tokenizer, messages, max_new_tokens=max_new_tokens
        )
    ...
    for i, sentence in enumerate(tqdm.tqdm(original_sentences)):
        ...
        for aspect in teacher_aspects_sentence:
            messages = prompt_syntactic_parsing(
                sentence=sentence, aspect=aspect, structure=conllu_parse
            )
            response = generate_response(
                student_model,
                student_tokenizer,
    ...
    for i, sentence in enumerate(tqdm.tqdm(original_sentences)):
        ...
        for j, aspect in enumerate(teacher_aspects_sentence):
            ...
            messages = prompt_opinion_extraction(
                sentence=sentence, aspect=aspect, syntactic_info=syntactic_context
            )
            response = generate_response(
                student_model,
                student_tokenizer,
    ...
    for i, sentence in enumerate(tqdm.tqdm(original_sentences)):
        ...
        for j, aspect in enumerate(teacher_aspects_sentence):
            ...
            messages = prompt_sentiment_classification(
                sentence, aspect, opinion_context
            )
            response = generate_response(
                student_model,
                student_tokenizer,
